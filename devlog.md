...
### 2026-09-25 · 大扫除 🧹：三模型交叉审计 → 76,679 行删除 + 功耗四件套（本地 commit，未 push 未发版）

- **起源与方法**：用户指令「代理一个子项目对项目全面框架梳理、优化、清理死代码，提升效率降低功耗」。grilling 三轮定边界：预防性大扫除（无特定痛点，审计驱动）；范围=App Kotlin+Python wrapper+构建配置（文档账册不动）；死代码豁免=未来模块可能用到的接口（adb 控制面切换/设置页日志区/ACCESSIBILITY 悬浮模式保留，方案 C 不保留）；节奏=零风险档直接动手、其余过目、分模块小 commit、push 逐次授权。
- **三方审计交叉查验**（用户指定）：kimi-code 与 deepseek-flash 与 gemini-3.8-flash-high 各出独立报告，再两两互评+主代理取证裁决。关键裁决：① `weights.npz` 3.4MB「APK 冗余去重」**撤销**——裁决当时 `build-rootfs.sh` 从不安装 `overlays/models/`（只装 PP-OCR 三件套，:217-219 是另一个目录），APK assets 经 AlasOverlay 是唯一交付路径，删=azur_lane numpy OCR 静默回落 PP-OCR 且无报警；G2 起镜像也装了模型，保留理由随之升级为「双源门禁+升级路径保险」（存量旧 rootfs 无模型，APK overlay 是其唯一升级路径）；② `app_name` 活（build-logic manifestPlaceholders），字符串扫描必须含 build-logic；③ `third/IO.java` 活（Command.java 在用），`StatusBarManager.java` 死；④ icons-extended 实为 7 种图标 10 处（core 无等价物，保留）；⑤ `MaaChoiceChip.kt` 只死 `MaaMultiChoiceFlow`（SingleChoiceFlow 在设置页 4 处活）；⑥ **AppWatchdog 从未启动**（v3 重写丢 `startWatching()`，`watchdogState()` 恒 IDLE，PermissionManager 还在 2s 空转轮询）——判定为 bug 级死代码，整链删（用户批准）。
- **批次 A–E 纯删除**（coder 执行，每批 assembleRelease+verifyReleaseR8Keeps+testDebugUnitTest 门禁）：`e453629` test 源集大扫除（54→6 测试文件，20 tests 复活转绿；潜伏的 ButtonPrimitiveBoundaryTest 一行最小修复）；`78568f6` 死 Kotlin 11 整文件+死成员+死依赖（androidx.window、ui-tooling-preview）+baselineProfiles 49k 行入库死文件+死 drawable/colors（colors.xml 整删）；`aec4612` 不可达链（前台模式/主屏分辨率/亮屏解锁/AIDL runner 残桩/屏保 537 行/AppWatchdog 链+2s 轮询，RemoteService.aidl 136→64 行删 21 方法只删不重排，DriverClass.java 签名未动）；`8bbbd6f` :semi-icons 模块（592 图标 2.7MB）+MaaIcons；`4151c00` 死字符串 428 组×双语+5 plurals（561 组中 428 零引用，排除 app_name）。合计 735 文件 +155/-76,679。
- **批次 F 功耗四件套**：`3e8d7ee` wrapper /status `log_lines`→`log_size`（原实现每 4s 逐行通读整份几十 MB 日志只为滚底 key；改 os.path.getsize O(1)，语义等价，双源 cmp 一致）；`f6eb30f` WebView 退后台 onPause+pauseTimers（新增 AlasWebViewHolder，MainActivity onStop/onStart 驱动；**切 tab 不暂停**，保 v0.1.5 pager 常驻决策）+ 空闲退避（新增 AppForegroundTracker/ProcessLifecycleOwner：环境未起且后台时桥 ping 与 wrapper 轮询 4s→30s，启动动作/回前台 poke 立即恢复 4s）+ 失败日志状态沿节流（首败一条/持续每 5min 一条/恢复一条——原关停态 20~40 万行/天带堆栈写盘）。
- **批次 G 质量基建**：`d6f03c7` App 侧首个 CI（app.yml：testDebugUnitTest→assembleDebug→双源 diff 门禁（--exclude=seeds 布局差异）→i18n 校验；fetch-depth: 0 因 GitVersion 靠 git describe）；`208d6b4` OCR 门禁盲区修复（build-rootfs.sh 补装 azur_lane weights+label+al_numpy 进镜像——原来 gate 的 azur_lane 用例恒走 PP-OCR 回落从未真验 numpy 引擎；gate 新增 `proxy._al is not None` 断言）；`1c76b0b` 文档/注释腐烂清扫（development.md 指针、proguard RunLogKind、BuildProfile dormant 头注、MaaMeow 误导注释×2、stage2-inventory 状态横幅）+`.tmp` 回收 7.2GB（gradle-home 未碰）。
- **真机回归**（0.1.6-alpha.11/79，设备 AVAY025422002864）：装机覆盖成功；环境自启正常；`/status` 已输出 `log_size`（F1 上线实证）；app.log 出现 `wrapper /status recovered after 1 failures`（F4 节流格式实证）。插曲：Shizuku-m 未运行导致桥 22300 refused、工具报 RequestHumanTakeover——非本次改动，App 正确显示「未运行」态；shizuku-m App 点「离线自连」后 shizuku_server 拉起、22300 监听、特权链自愈。三件套全过：半自动点击 daemon 存活并自行拉起游戏（VD id=46，1280x720 截图链正常）；活动剧情 event_story 完整跑通（page_event→切 story 模式→finish 自然退出）；挂机 /start runner_alive。切页 18 次（6 轮 ALAS/设置/挂机）「启动环境/环境未启动」0 闪；HOME 退后台 35s 回切挂机无损、UI 无回退。**15min 挂机观察**（18:41→18:57）：runner 全程 alive、respawns=0、正常刷图（OCR_COIN 95921→96831 金币在涨）、停止干净（SIGTERM -15）；电量 99%→100%（设备在充电，功耗对比口径不可用，仅记稳定性）。
- **遗留观察**：热更新 CDN 拉包成功但 `git reset --hard` FAILED（unable to read tree）+ env_fix `template.py WARN restore-failed`——均为预存现象（非本次改动面），记入待查。F3 的 30s 退避支路（冷态+后台）本次回归未覆盖（挂机中 VD 存续不满足触发条件），仅构建门禁+代码审查兜底。
- **共识表决与收口**：kimi-code / gemini-3.8-flash-high 无保留通过；deepseek-flash 有保留共识，7 条非阻断保留项全部闭环：R1/R2 账册勘误（测试实为 **20** 个、AIDL 删 **21** 方法/136→**64** 行）；R3 proguard 残留 keep（IMaaRunnerCallback）删除；R4 死文档指针注释修正；R6 CI 双源门禁补守 seeds 对（`--exclude=deploy.yaml`）；R7 `AlasRunController.get` 非 200 分支先记 `lastGetFailure` 再 return。**R5 WebView `pauseTimers()` 进程全局语义**：Activity 重建（切语言）后新 WebView 继承残留全局暂停→永久卡 Loading（真机已复现）；修复=`AlasWebViewHolder` 加 `paused` 标志位、`register()` 按标志对齐。回归实证：English→简体中文重建后 pywebio 控制台正常渲染（修复前此场景卡死），HOME 35s 回切无损（正常退后台路径未破坏）。**意外收获 R8**：R4 落地时发现 `.gitignore` 裸模式 `config/` 把 `com.aliothmoon.maafw.config` 整包挡在仓库外（`UserConfigurationStore.kt` 从未入库——本地能编、fresh clone 必炸），锚定 `/config/` + 补入库；新建的 app.yml CI 尚未首跑即提前立功。收口 commit `2c7a9e6`。
- **改进方案交付**：`docs/cleanup-consensus-2026-09-25.md`（三方审计方法/关键裁决表/批次数据/R1–R8 闭环/缓办清单+理由/后续路线图）。
- 至此本地累计 13 个 commit（10 代码 + 3 账册/收口），**未 push**（等用户授权）；未发版。
