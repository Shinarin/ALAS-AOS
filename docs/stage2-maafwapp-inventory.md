...
1. **桥不在 fork 里，"保留 5 端点"是重写不是删代码。** m0 的桥 = Python 写的 MaaFramework Agent（`m0-archive/spike/m0/agent/main.py`），跑在特权进程 fork 的 agent child 里，协议 = 行分隔 JSON + 二进制帧的裸 TCP 127.0.0.1:22300（**非 HTTP**）。端点实现：`ping/screencap/click/swipe/ocr/shell` 共 6 个（`main.py:148-232`），`ocr` 走 MaaFramework PP-OCR（剔除项），其余经 `context.tasker.controller` → libMaaFramework → libMaaAndroidNativeControlUnit → dlopen `libbridge.so`。删 libMaaCore 会连桥本体一起删掉。
...
3. **钉版点带 6 处未提交改动 = m0 WebView 资产，复活前必须固化。** 新增 `ui/alas/AlasScreen.kt`、`res/xml/network_security_config.xml`；改 `AndroidManifest.xml`、`ui/AppRoot.kt`、`ui/navigation/Routes.kt`、`res/values{,-en}/strings.xml`。无 commit/branch/tag，`git clean` 即丢。
...
| `:semi-icons` | 保留（UI 图标） |
| `:macrobenchmark` | 可选剔除 |
| `:build-logic/convention` | 保留，拆掉 `maafw.pi.assets` 与 `maafw.agent.runtime` 两个插件 |

### app 包（com.aliothmoon.maafw）
| 包 | 职责 | 归类 |
|---|---|---|
| `bridge/`（4 文件） | JNI 入口/DriverClass/InputControlUtils/TouchPointerSequence | **保留** |
| `maa/`（4） | JNA 声明 MaaFramework C API | **剔除** |
| `remote/`（20） | 特权进程侧 | **混合**：`internal/` 与 AIDL 面保留；`MaaRunner`/`ExecAgentHost`/`AgentInstaller`/`AgentRuntimeDescriptor` 剔除 |
| `privileged/`（20） | Shizuku/Root 授权绑定、权限代授、安装辅助 | **保留** |
| `root/`（6） | Root 后端 binder 回投 | 保留（可选删） |
| `runner/`（29） | MaaFramework 业务管线 | **剔除主体**；`RunnerPort` 接口被 UI/悬浮窗/前台服务依赖，需留壳或退化 |
...
- `ui/alas/AlasScreen.kt`：URL `127.0.0.1:22267`（`:40`）；vh 塌缩修复注入（`:49-61,116`）；JS/domStorage（`:96-97`）；错误页拉起按钮当前指向 `com.termux`（`:169-172`）→ **改拉起 rootfs 侧服务**。
...
- **坑①：`F/local.properties:2` `pi.profile=D:/VSCodeCache/maa-alas/pi-profile.m0.yaml` 是死路径**（文件已归档 `m0-archive/`），`BuildProfile.kt:91-92` 硬失败 → 构建前先删该行（无 PI 仅 warn）或指新路径。
- 坑②：jniLibs 新 clone 为空（原流程 `scripts/setup_maa_framework.py` 拉 v5.12.3）——减法后不需要。
